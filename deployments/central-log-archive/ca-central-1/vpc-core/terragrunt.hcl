...
dependencies {
  paths = [
    "../log-archive",
    format("%s/network-hub/ca-central-1/transit-gateway", "${get_parent_terragrunt_dir()}"),
    format("%s/network-hub/ca-central-1/vpc-internet-egress", "${get_parent_terragrunt_dir()}"),
  ]
}

dependency "log_archive" {
  config_path = "../log-archive"
  mock_outputs = {
...
inputs = {
  ...
  log_archive_outputs         = dependency.log_archive.outputs
  deployment_tags = {
